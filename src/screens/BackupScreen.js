...
const BackupScreen = () => {
        ...
        {text: 'Cancel', style: 'cancel'},
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              // Use copied file path for reliable reading on Android
  ...
  const handleFullPDF = async () => {
    ...
    try {
      ...
      const lena = summary.totalToReceive || 0;
      const dena = summary.totalToPay || 0;
      ...
      const contactRows = contacts
        .map(c => {
          ...
          return `<tr>
            ...
            <td>${c.phone || '—'}</td>
            <td class="${balClass}">${formatMoney(Math.abs(bal))}</td>
      ...
      const txnRows = transactions
        ...
        .map(t => {
          ...
          return `<tr>
            ...
            <td>${t.note || '—'}</td>
            <td class="${isGave ? 'negative' : 'positive'}">${formatMoney(t.amount)}</td>
          </tr>`;
        })
        .join('');
...
<body>
  ...
  <div class="summary">
    <div class="summary-box green-box">
      ...
      <div class="val" style="color:#2ecc71">${formatMoney(lena)}</div>
    </div>
    <div class="summary-box red-box">
      <div class="label">Total Dena Hai</div>
      <div class="val" style="color:#e74c3c">${formatMoney(dena)}</div>
    </div>
